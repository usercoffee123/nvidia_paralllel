...
def run_distributed(rows, qubits: int, layers: int, num_gpus: int = None):
    ...
    print(f"Coordinator: Using {num_workers} GPU(s): {visible_gpus}", flush=True)
    ...
    worker_procs = []
    for worker_id, gpu_id in enumerate(visible_gpus):
        cmd = [
            sys.executable,
        ...
        worker_procs.append((worker_id, gpu_id, proc))
        print(f"Spawned worker {worker_id} on GPU {gpu_id} (PID {proc.pid})", flush=True)
    
    def fail_dead_workers():
        """Raise if any worker process exited; their messages would never arrive."""
        for w, g, p in worker_procs:
            if p.poll() is not None:
                out = p.stdout.read() if p.stdout else ""
                raise RuntimeError(
                    f"Worker {w} (GPU {g}) died (exit {p.returncode}). Output:\n{out}")
    ...
    while len(ready) < num_workers:
        ...
        if time.monotonic() > deadline:
            raise RuntimeError(f"Only {len(ready)}/{num_workers} workers ready after 120s: {ready}")
        try:
        ...
        if meta.get("status") == "ready":
            ...
            print(f"Ready {len(ready)}/{num_workers}: {frames[0].decode()}", flush=True)
    
    # Divide rows into chunks (one per worker)
    ...
    print(f"Divided {len(rows)} rows into {num_workers} chunks of ~{chunk_size} rows each", flush=True)
    
    # Send work to each worker via ROUTER
    # ROUTER addresses workers by identity: b"worker-0", b"worker-1", etc.
    # Payload is binary float64 (JSON for 100k+ rows takes minutes to encode/parse)
    print("Sending work to all workers...", flush=True)
    for i in range(num_workers):
        identity = f"worker-{i}".encode()
        ...
        router_socket.send_multipart(
            [identity, b"", json.dumps(meta).encode(), chunk.tobytes()])
    
    # Collect results from workers via ROUTER
    print("Collecting results...", flush=True)
    results_by_chunk = [None] * num_workers
    collected = 0
    ...
    for worker_id, gpu_id, proc in worker_procs:
        try:
            proc.wait(timeout=10)
            stdout_data = proc.stdout.read() if proc.stdout else ""
            for line in stdout_data.split("\n"):
                if line.strip():
                    print(f"  [W{worker_id}/GPU {gpu_id}] {line.strip()}", flush=True)
        except subprocess.TimeoutExpired:
            print(f"Worker {worker_id} (GPU {gpu_id}) timed out, terminating...", flush=True)
            proc.terminate()
            try:
                proc.wait(timeout=5)
            except subprocess.TimeoutExpired:
